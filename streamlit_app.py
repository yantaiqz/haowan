...
def render_home():
    ...
    for idx, (title, desc, icon, url) in enumerate(current_text['games']):
        with cols[idx % 3]:
            ...
            # 生成唯一的按钮ID
            btn_id = f"click_btn_{url.replace('https://', '').replace('/', '_').replace('.', '_')}"
            
            # 隐藏的点击按钮（核心：通过Streamlit原生按钮触发点击记录）
            if st.button(" ", key=btn_id, class_="hidden-click-btn"):
                record_click(url)
                # 触发页面刷新以更新计数
                st.rerun()
            ...
            # 游戏卡片 - 点击时先触发隐藏按钮，再跳转
            st.markdown(f"""
            <a href="javascript:void(0);" onclick="document.querySelector('[data-testid=\"stButton\"][key=\"{btn_id}\"] button').click(); setTimeout(() => window.open('{url}', '_blank'), 200);" class="card-link">
                <div class="neal-card">
